  ...
  >
    ...
    >
      <component
        ...
        :href="link.path"
      >
        <template v-if="addLink" style="margin-left:20px !important;">
          <md-icon v-if="link.icon" style="font-size: 20px !important; margin-top: -3px; margin-right: 19px !important; margin-left: 20px !important;">{{ linkPrefix }}</md-icon>
          <span v-else class="sidebar-mini" style="margin-left:20px !important">{{ linkPrefix }}</span>
...
export default {
  ...
  computed: {
    ...
    linkPrefix() {
      if (this.link.name && !this.link.icon) {
        let words = this.link.name.split(" ");
        return words.map(word => word.substring(0, 1)).join("");
      } else if(this.link.icon) {
